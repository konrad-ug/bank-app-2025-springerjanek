...
Scenario: User is able to update name of already created account
    Given Account registry is empty
    And I create an account using name: "alex", last name: "turner", pesel: "88010112345"
    When I update "name" of account with pesel: "88010112345" to "alexander"
    Then Account with pesel "88010112345" has "name" equal to "alexander"

Scenario: Created account has all fields correctly set
    Given Account registry is empty
    When I create an account using name: "amy", last name: "winehouse", pesel: "83091456789"
    Then Account with pesel "83091456789" has "name" equal to "amy"
    And Account with pesel "83091456789" has "surname" equal to "winehouse"
    And Account with pesel "83091456789" has "pesel" equal to "83091456789"
